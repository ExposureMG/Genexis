...
void Document::add_comment(std::string_view section_name,
                           std::string_view comment, char comment_char) {
  std::string sec_key = ToLower(std::string(section_name));
  sections[sec_key].push_back(MakeComment(comment, comment_char));
}
  ...
  while (!remaining.empty()) {
    ...
    if (line.front() == ';' || line.front() == '#') {
      if (!current_section.empty()) {
        doc.sections[current_section].push_back(
...
std::string Serialize(const Document &doc, bool xebuild_format) {
  ...
  for (const auto &[section_name, entries] : doc.sections) {
    ...
    for (const auto &entry : entries) {
      std::string_view trimmed_key = Trim(entry.key);
      if (!trimmed_key.empty() &&
          (trimmed_key.front() == ';' || trimmed_key.front() == '#')) {
        ss << entry.key << "\n";
        continue;
      }
      ...
      if (entry.key.empty()) {
        if (!entry.comment.empty()) {
          std::string_view trimmed_comment = Trim(entry.comment);
          if (!trimmed_comment.empty() && trimmed_comment.front() != ';' &&
              trimmed_comment.front() != '#') {
            ss << "; " << entry.comment << "\n";
          } else {
            ss << entry.comment << "\n";
          }
...
Entry MakeEntry(std::string_view key, std::string_view value,
                  std::string_view hash, std::string_view comment) {
  return Entry{
      .key = std::string(key),
      .value = std::string(value),
      .hash = std::string(hash),
...
Entry MakeComment(std::string_view comment, char comment_char) {
  std::string prefix;
  if (comment_char != '#' && comment_char != ';') {
    comment_char = ';';
  }
  std::string trimmed_comment = std::string(Trim(comment));
  if (!trimmed_comment.empty() && trimmed_comment.front() != ';' &&
      trimmed_comment.front() != '#') {
    prefix = std::string(1, comment_char) + " ";
  }
  return Entry{
      .key = prefix + trimmed_comment,
  };
}
