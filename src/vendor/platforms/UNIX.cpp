...
std::vector<RawUSBDevice> UnixUSBDiscovery::getAllDevices() {
  std::vector<RawUSBDevice> devices;
#if defined(__linux__)
  std::string sysUsbDir = "/sys/bus/usb/devices";
  std::error_code ec;
  if (!std::filesystem::exists(sysUsbDir, ec)) {
    return devices;
  }

  auto readSysFile = [](const std::filesystem::path &p) -> std::string {
    std::ifstream file(p);
    if (!file.is_open())
      return "";
    std::string line;
  ...
  for (const auto &entry : std::filesystem::directory_iterator(sysUsbDir, ec)) {
    ...
    std::string vendorHex = readSysFile(devPath / "idVendor");
    std::string productHex = readSysFile(devPath / "idProduct");

    if (vendorHex.empty() || productHex.empty()) {
      continue;
    }

    std::transform(vendorHex.begin(), vendorHex.end(), vendorHex.begin(),
                   ::tolower);
    std::transform(productHex.begin(), productHex.end(), productHex.begin(),
                   ::tolower);

    RawUSBDevice dev;
    dev.vendorHex = vendorHex;
    dev.productHex = productHex;

    try {
      dev.vendorId = static_cast<uint16_t>(std::stoul(vendorHex, nullptr, 16));
      dev.productId =
          static_cast<uint16_t>(std::stoul(productHex, nullptr, 16));
    } catch (...) {
    }

    dev.manufacturer = readSysFile(devPath / "manufacturer");
    dev.productName = readSysFile(devPath / "product");
    dev.serialNumber = readSysFile(devPath / "serial");
    dev.devicePath = devPath.string();

    std::string devClass = readSysFile(devPath / "bDeviceClass");
    dev.isMassStorage = (devClass == "08");
