...
mod ffi {
    ...
    unsafe extern "RustQt" {
        ...
        type GxBuild = super::GxBuildSession;

        #[qinvokable]
        fn run_build(self: Pin<&mut GxBuild>) -> QString;

        #[qinvokable]
        fn set_ini_dir(self: Pin<&mut GxBuild>, path: QString);
        
        #[qinvokable]
        fn set_data_dir(self: Pin<&mut GxBuild>, path: QString);
    }
}

pub struct GxBuildSession {
    build_type: QString,
...
impl Default for GxBuildSession {
    fn default() -> Self {
        Self {
    ...
    pub fn run_build(mut self: Pin<&mut Self>) -> QString {
        ...
        if let Ok(mut session) = session_arc.lock() {
            ...
            if let Some(err) = &session.last_error {
                ...
                self.as_mut().set_status(QString::from(format!("Error: {}", err)));
                return err_qs;
            }
        }
