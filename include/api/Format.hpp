...
// Unified interface over host directories, NAND images, STFS/PEC/XCP containers,
// FATX partitions, and Xbox 360 boot-file metadata extraction.
//
...
struct BootloaderMeta {
    std::string   bl_name;           // "CB", "CB_A", "CB_X", "CB_B", "SC", "CD", "SD", "CE", "SE"
    std::uint16_t version{0};        // bl_header.version
    std::uint8_t  lockdown_value{0}; // LDV — from more_globals[1] in bl2_header (when decrypted)
...
struct KeyvaultMeta {
    ...
    std::string  dvd_key_hex;        // 32 hex chars (EG. 13319182EBA65CCB1D2F63C184D4FE2E)
...
// Future: add new metadata structs above this comment, then add them to the variant.

using FileMetadata = std::variant<
    GenericFileMeta,
    BootloaderMeta,
    CfMeta,
    CgMeta,
    ...
    SpaMeta,
    XdbfMeta
    // Future: add new types to the variant list above (e.g. GodMeta, SvodMeta …)
>;
...
public:
    ...
    [[nodiscard]] static std::optional<FsContext>
    openFatx(const std::filesystem::path& image,
    ...
    probe(std::string_view path) const;

    // Static utility: probe a raw blob without an open context.
    // hint_name is the filename (used only to break ambiguity between GPD/SPA/XDBF).
    [[nodiscard]] static std::optional<FileMetadata>
    probeBlob(std::span<const std::byte> data,
              std::string_view hint_name = "");

    // Read the raw bytes of a file at path.
    [[nodiscard]] std::vector<std::byte>
    read(std::string_view path) const;

    // Write data to path.  Returns false for read-only containers (STFS/PEC/XCP/FlashFs/Nand).
